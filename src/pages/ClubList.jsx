    ...
    return(
        <div className="min-h-screen">
            ...
            <div className={viewMode === "grid" ? "grid grid-cols-3 gap-4 justify-center m-4" : "flex flex-col gap-2 max-w-xl mx-auto"}>
                {clubs.map((club) =>(
                    <div key={club.id} className="flex flex-col border p-6 gap-2">
                        <h2 className="text-2xl">{viewMode === "grid" ? "Grid" : "List"} {club.name}</h2>
                        <p>{club.shortDescription}</p>
                        <img src={club.image} alt="Club photos" className=" h-full"/>
                        <button className="bg-amber-500 rounded-xl p-3 text-white hover:bg-black hover:cursor-pointer">See more</button>
                        {/* <p>{}</p> */}
                    </div>
                ))}
            </div>
